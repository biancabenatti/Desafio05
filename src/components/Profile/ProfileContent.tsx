...
const ProfileContent: React.FC<ProfileContentProps> = ({ repos }) => {
  const [sortedRepos, setSortedRepos] = useState<Repository[]>(repos); // Tipando o estado
  const [filter, setFilter] = useState<string>('stars-desc'); // Tipando o estado

  useEffect(() => {
    const sortRepos = (repos: Repository[]) => { // Tipando o parâmetro
      switch (filter) {
        case 'stars-asc':
